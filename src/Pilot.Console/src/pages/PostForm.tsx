...
export default function PostFormPage() {
  ...
  const [scheduledAt, setScheduledAt] = React.useState(() => {
    if (!isEditing) {
      ...
      return toDatetimeLocal(oneHourFromNow);
    }
    return '';
  });
  const [existingImages, setExistingImages] = React.useState<string[]>([]);
  const [existingImageItems, setExistingImageItems] = React.useState<Array<{ id: string; url: string }>>([]);
  const [newFiles, setNewFiles] = React.useState<File[]>([]);
  const [status, setStatus] = React.useState<'Draft' | 'Scheduled' | 'Posted' | 'Failed'>('Draft');
    ...
    try {
      ...
      if (isEditing && postId) {
        ...
        if (postRes.data) {
          ...
          setExistingImages(postRes.data.mediaUrls || []);
          setStatus(postRes.data.status as any);
        } else {
  ...
  React.useEffect(() => {
    setExistingImageItems(prev => {
      ...
      for (const url of existingImages) {
        const matchIndex = available.findIndex(item => item.url === url);
        if (matchIndex >= 0) {
          next.push(available[matchIndex]);
  ...
  const handleRemoveExistingImage = (id: string) => {
    setExistingImageItems(prev => prev.filter(item => item.id !== id));
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-muted-foreground/10 bg-card shadow-sm overflow-hidden rounded-2xl">
            <CardHeader className="bg-muted/30 border-b border-muted-foreground/5">
              ...
              <div className="grid gap-4">
                ...
                {(existingImageItems.length > 0 || newFiles.length > 0) ? (
                  <div 
                    ...
                    onDrop={handleDrop}
                    className={cn(
                      "space-y-3 p-2 rounded-2xl transition-all relative min-h-[100px]",
                    ...
                    {isDragging ? (
                      ...
                      <>
                            ...
                            >
                              {existingImageItems.map((item) => (
                                <Reorder.Item
                                  key={item.id}
                                  value={item}
                                  className="flex items-center gap-4 p-3 rounded-xl border border-muted-foreground/10 bg-muted/30 group shadow-sm"
                        ...
                        {newFiles.map((file, index) => (
                          <div
                            key={`new-${index}`}
                            className="flex items-center gap-4 p-3 rounded-xl border border-primary/20 bg-primary/5 group shadow-sm"
                          >
                            ...
                            <div className="flex-1 truncate text-xs text-foreground font-medium">
                              {file.name}
                            </div>
                            {file.type.startsWith('video/') && (
                              <Badge variant="secondary" className="text-[10px] uppercase">Video</Badge>
                            )}
              ...
              <div className="grid gap-3">
                ...
                <div className="relative">
                  ...
                  <Input
                    id="scheduledAt"
                    type="datetime-local"
                    className="pl-10 h-11 border-muted-foreground/10 focus:border-primary rounded-xl bg-muted/20"
                    value={scheduledAt}
